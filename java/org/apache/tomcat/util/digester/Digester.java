 ...
 * <p>
 * Digester通过匹配一系列元素嵌套模式来处理XML输入流，以执行在解析开始之前添加的规则。
 * 这个包的灵感来自Tomcat 3.0和3.1中的XmlMapper类，但是组织方式有所不同。有关更多信息，
 * 请参阅Digester开发人员指南。
 * 实现注意:单个Digester实例一次只能在单个线程的上下文中使用，即使在同一个线程中，也必须先完成对parse()的调用
 * ，然后才能启动另一个。实现注意:Xerces 2.0.2中的一个bug阻止了对XML schema的支持。
 * 您需要Xerces 2.1/2.3及以上版本才能使该类使用XML schema
 */
